...
const StageMedia = (record) => {
  ...
  // Helper function to check if a row is overdue
  const isOverdue = (record) => {
    const stage2Date = record?.payment?.stage2?.date ? moment(record.payment.stage2.date) : null;
    const today = moment();
    const isDateOverdue = stage2Date && today.diff(stage2Date, 'days') > 7;
    const isStage2NotDone = !record?.stage2Completion || record?.stage2Completion !== 'Done';

    return isDateOverdue && isStage2NotDone;
  };
